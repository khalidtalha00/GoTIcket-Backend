...
const findBestStationMatch = (candidateName, stationNames) => {
  const normalizedCandidate = normalizeStationName(candidateName);
  if (!normalizedCandidate) return null;

  const exact = stationNames.find(
    (name) => normalizeStationName(name) === normalizedCandidate
  );
  if (exact) return exact;

  const includeMatch = stationNames.find((name) => {
...
const getNearbyMatchedStations = async (lat, lng, stationNames) => {
  const overpassQuery = `
    [out:json];
    (
      node(around:3000,${lat},${lng})["railway"~"station|halt"]["station"~"subway|light_rail",i];
      node(around:3000,${lat},${lng})["public_transport"="station"]["subway"="yes"];
      node(around:3000,${lat},${lng})["name"~"metro",i];
    );
    out center 25;
  `;
  ...
  if (!response.ok) {
    const err = new Error('Failed to fetch nearby metro stations');
    err.statusCode = 502;
    throw err;
  }
  ...
  const nearby = elements
    ...
    .map((item) => ({
      ...
      distanceKm: Math.sqrt(
        ...
        Math.pow((item.lng - lng) * 111 * Math.cos((lat * Math.PI) / 180), 2)
      ),
    }))
  ...
  return nearby
    .map((item) => {
      const match = findBestStationMatch(item.name, stationNames);
      return match
        ? {
...
router.get('/nearest', async (req, res) => {
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
    return res.status(400).json({ status: 400, error: 'Invalid coordinates' });
  }

  const stationNames = [...new Set(g.nodes.map((n) =>
    n.split(' ').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')
  ))];

  try {
    const matched = await getNearbyMatchedStations(lat, lng, stationNames);

    if (matched.length === 0) {
      return res.status(404).json({ status: 404, error: 'No nearby metro station found' });
    }

    return res.json({
      status: 200,
      station: matched[0].stationName,
      matchedFrom: matched[0].sourceName,
      distanceKm: Number(matched[0].distanceKm.toFixed(2)),
    });
  } catch (error) {
    console.error('Failed to resolve nearest metro station', error);
    const statusCode = error?.statusCode || 500;
    return res.status(statusCode).json({ status: statusCode, error: 'Failed to resolve nearest metro station' });
  }
});
...
router.get('/nearby', async (req, res) => {
  ...
  try {
    const matched = await getNearbyMatchedStations(lat, lng, stationNames);
    const uniqueByStation = [];
    const seen = new Set();

    for (const item of matched) {
      if (!seen.has(item.stationName)) {
        seen.add(item.stationName);
        uniqueByStation.push(item);
      }
      if (uniqueByStation.length >= limit) break;
    }

    return res.json({
      status: 200,
      stations: uniqueByStation,
    });
  } catch (error) {
    console.error('Failed to resolve nearby metro stations', error);
    const statusCode = error?.statusCode || 500;
    return res.status(statusCode).json({ status: statusCode, error: 'Failed to resolve nearby metro stations' });
  }
});

router.get('/route', (req, res) => {
  if(!req.query.to || !req.query.from) {
    return res.status(400).json({'status' : 400, 'error': 'Invalid params'});
  }
  
  let to = req.query.to.toLowerCase();
  let from = req.query.from.toLowerCase();
  let result = g.shortestRoute(from, to);
  
  if (result.status === 200 && result.time) {
    // Fare calculation
    const distance = result.time * 0.55;
    let fare = 0;
